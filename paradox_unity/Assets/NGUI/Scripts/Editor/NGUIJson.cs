...
// Source: NGUIToolkit -- https://github.com/prime31/NGUIToolkit/blob/master/Assets/Plugins/MiniJSON.cs
...
public class NGUIJson
{
	...
	public static void LoadSpriteData (NGUIAtlas atlas, TextAsset asset)
	{
		...
		List<NGUIAtlas.Sprite> oldSprites = atlas.spriteList;
		atlas.spriteList = new List<NGUIAtlas.Sprite>();
		...
		foreach (System.Collections.DictionaryEntry item in frames)
		{
			...
			foreach (NGUIAtlas.Sprite oldSprite in oldSprites)
			{
	...
	static int CompareSprites (NGUIAtlas.Sprite a, NGUIAtlas.Sprite b) { return a.name.CompareTo(b.name); }

	/// <summary>
	/// Copy the inner rectangle from one sprite to another.
	/// </summary>

	static void CopyInnerRect (NGUIAtlas.Sprite oldSprite, NGUIAtlas.Sprite newSprite)
	{
