...
static public class NGUIMenu
{
		...
		if (go != null)
		{
			...
			if (sprite.atlas != null)
			{
				...
				if (sp != null)
				{
					...
					if (sp.inner != sp.outer) sprite.type = NGUISprite.Type.Sliced;
				}
			}
	...
	static public void AddAnchor ()
	{
		...
		if (go != null)
		{
			...
			if (go.GetComponent<NGUIAnchor>() == null) go.AddComponent<NGUIAnchor>();
		}
		else
		{
			Debug.Log("You must select a game object first.");
	...
	{
		EditorWindow.GetWindow<NGUICreateWidgetWizard>(false, "Widget Tool", true);
	}

	[MenuItem("NGUI/Open the UI Wizard")]
	static public void CreateUIWizard ()
	{
		EditorWindow.GetWindow<NGUICreateNewUIWizard>(false, "UI Tool", true);
	}

	[MenuItem("NGUI/Open the Panel Tool")]
	static public void OpenPanelWizard ()
	{
		EditorWindow.GetWindow<NGUIPanelTool>(false, "Panel Tool", true);
	}

	[MenuItem("NGUI/Open the Camera Tool")]
	static public void OpenCameraWizard ()
	{
		EditorWindow.GetWindow<NGUICameraTool>(false, "Camera Tool", true);
	}

	[MenuItem("NGUI/Open the Font Maker #&f")]
	static public void OpenFontMaker ()
	{
		EditorWindow.GetWindow<NGUIFontMaker>(false, "Font Maker", true);
	}

	[MenuItem("NGUI/Open the Atlas Maker #&m")]
	static public void OpenAtlasMaker ()
	{
		EditorWindow.GetWindow<NGUIAtlasMaker>(false, "Atlas Maker", true);
	}
