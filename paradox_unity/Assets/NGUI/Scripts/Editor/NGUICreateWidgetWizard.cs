...
public class NGUICreateWidgetWizard : EditorWindow
{
	...
	static NGUIScrollBar.Direction mScrollDir = NGUIScrollBar.Direction.Horizontal;
	...
	static void Load ()
	{
		...
		mScrollDir = (NGUIScrollBar.Direction)EditorPrefs.GetInt("NGUI Scroll Dir", 0);
	...
	{
		NGUISettings.font = obj as NGUIFont;
		Repaint();
	}
	...
	void OnGUI ()
	{
		...
		if (go == null)
		{
			...
			{
				EditorWindow.GetWindow<NGUICreateNewUIWizard>(false, "New UI", true);
			}
		}
		else
		{
			...
			GUILayout.BeginHorizontal();
			ComponentSelector.Draw<NGUIAtlas>(NGUISettings.atlas, OnSelectAtlas, GUILayout.Width(140f));
			GUILayout.Label("Texture atlas used by widgets", GUILayout.MinWidth(10000f));
			GUILayout.EndHorizontal();

			GUILayout.BeginHorizontal();
			ComponentSelector.Draw<NGUIFont>(NGUISettings.font, OnSelectFont, GUILayout.Width(140f));
