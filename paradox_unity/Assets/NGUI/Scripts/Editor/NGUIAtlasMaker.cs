...
public class NGUIAtlasMaker : EditorWindow
{
	class SpriteEntry
	{
	...
	{
		NGUISettings.atlas = obj as NGUIAtlas;
		Repaint();
	}
	...
	static NGUIAtlas.Sprite AddSprite (List<NGUIAtlas.Sprite> sprites, SpriteEntry se)
	{
		NGUIAtlas.Sprite sprite = null;

		// See if this sprite already exists
		foreach (NGUIAtlas.Sprite sp in sprites)
		{
	...
	static void ReplaceSprites (NGUIAtlas atlas, List<SpriteEntry> sprites)
	{
		// Get the list of sprites we'll be updating
		List<NGUIAtlas.Sprite> spriteList = atlas.spriteList;
		List<NGUIAtlas.Sprite> kept = new List<NGUIAtlas.Sprite>();

		// The atlas must be in pixels
		atlas.coordinates = NGUIAtlas.Coordinates.Pixels;

		// Run through all the textures we added and add them as sprites to the atlas
		for (int i = 0; i < sprites.Count; ++i)
		{
			SpriteEntry se = sprites[i];
			NGUIAtlas.Sprite sprite = AddSprite(spriteList, se);
			kept.Add(sprite);
		}
		...
		{
			NGUIAtlas.Sprite sp = spriteList[--i];
	...
	static void ExtractSprites (NGUIAtlas atlas, List<SpriteEntry> sprites)
	{
		...
		if (atlasTex != null)
		{
			...
			List<NGUIAtlas.Sprite> list = atlas.spriteList;

			foreach (NGUIAtlas.Sprite asp in list)
			{
				bool found = false;
	...
	static bool UpdateTexture (NGUIAtlas atlas, List<SpriteEntry> sprites)
	{
		...
		else
		{
			...
			EditorUtility.DisplayDialog("Operation Canceled", "The selected sprites can't fit into the atlas.\n" +
					"Keep large sprites outside the atlas (use NGUITexture), and/or use multiple atlases instead", "OK");
			return false;
		}
	}
	...
	static public void AddOrUpdate (NGUIAtlas atlas, Texture2D tex)
	{
	...
	void UpdateAtlas (List<Texture> textures, bool keepSprites)
	{
		...
		if (sprites.Count > 0)
		{
			...
			//NGUIEditorTools.RegisterUndo("Update Atlas", NGUISettings.atlas, NGUISettings.atlas.texture, NGUISettings.atlas.material);
	...
	void OnGUI ()
	{
		...
		if (NGUISettings.atlas == null && go != null) NGUISettings.atlas = go.GetComponent<NGUIAtlas>();
		...
		if (create)
		{
				...
				NGUISettings.atlasName + " atlas with the textures currently selected in the Project View? All other sprites will be deleted.", "Yes", "No"))
			{
				...
				if (NGUISettings.atlas == null || NGUISettings.atlas.name != NGUISettings.atlasName)
				{
					...
					go.AddComponent<NGUIAtlas>().spriteMaterial = mat;
					...
					NGUISettings.atlas = go.GetComponent<NGUIAtlas>();
				}
			}
		}

		ComponentSelector.Draw<NGUIAtlas>("Select", NGUISettings.atlas, OnSelectAtlas);
