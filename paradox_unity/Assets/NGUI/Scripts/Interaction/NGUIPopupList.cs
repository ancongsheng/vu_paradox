...
public class NGUIPopupList : MonoBehaviour
{
	...
	static public NGUIPopupList current;
	...
	NGUIPanel mPanel;
	GameObject mChild;
	NGUISprite mBackground;
	NGUISprite mHighlight;
	NGUILabel mHighlightedLabel = null;
	List<NGUILabel> mLabelList = new List<NGUILabel>();
	float mBgBorder = 0f;
	...
	bool handleEvents
	{
		get
		{
			NGUIButtonKeys keys = GetComponent<NGUIButtonKeys>();
	...
	void OnItemPress (GameObject go, bool isPressed) { if (isPressed) Select(go.GetComponent<NGUILabel>(), true); }
