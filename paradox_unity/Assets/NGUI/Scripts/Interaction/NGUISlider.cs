...
public class NGUISlider : IgnoreTimeScale
{
	...
	Transform mTrans;
	Transform mFGTrans;
	NGUIWidget mFGWidget;
	NGUISprite mFGFilled;
	...
	void Start ()
	{
		...
		{
			NGUIEventListener listener = NGUIEventListener.Get(thumb.gameObject);
	...
	void OnPress (bool pressed) { if (pressed && NGUICamera.currentTouchID != -100) UpdateDrag(); }
	...
	void UpdateDrag ()
	{
		...
		if (mCol == null || NGUICamera.currentCamera == null || NGUICamera.currentTouch == null) return;

		// Don't consider the slider for click events
		NGUICamera.currentTouch.clickNotification = NGUICamera.ClickNotification.None;
