...
public class NGUIScrollBar : MonoBehaviour
{
	...
	public delegate void OnScrollBarChange (NGUIScrollBar sb);
	public delegate void OnDragFinished ();

	[HideInInspector][SerializeField] NGUISprite mBG;
	[HideInInspector][SerializeField] NGUISprite mFG;
	...
	public NGUISprite background { get { return mBG; } set { if (mBG != value) { mBG = value; mIsDirty = true; } } }

	/// <summary>
	/// Sprite used for the foreground.
	/// </summary>

	public NGUISprite foreground { get { return mFG; } set { if (mFG != value) { mFG = value; mIsDirty = true; } } }
	...
	void OnPressBackground (GameObject go, bool isPressed)
	{
		...
		Reposition(NGUICamera.lastTouchPosition);
	...
	{
		mCam = NGUICamera.currentCamera;
		Reposition(mScreenPos + NGUICamera.currentTouch.totalDelta);
	}
	...
	void Start ()
	{
		...
		{
			NGUIEventListener listener = NGUIEventListener.Get(background.gameObject);
		...
		{
			NGUIEventListener listener = NGUIEventListener.Get(foreground.gameObject);
