...
[RequireComponent(typeof(NGUIInput))]
[AddComponentMenu("NGUI/Interaction/Input Validator")]
public class NGUIInputValidator : MonoBehaviour
{
	...
	void Start () { GetComponent<NGUIInput>().validator = Validate; }
	...
	char Validate (string text, char ch)
	{
        ...
        else if (logic == Validation.ADDTrim)
        {
            if (!ch.Equals(' '))
            {
                return ch;
