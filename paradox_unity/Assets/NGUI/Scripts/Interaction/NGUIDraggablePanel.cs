...
/// <summary>
/// This script, when attached to a panel allows dragging of the said panel's contents efficiently by using NGUIDragPanelContents.
/// </summary>

[ExecuteInEditMode]
[RequireComponent(typeof(NGUIPanel))]
[AddComponentMenu("NGUI/Interaction/Draggable Panel")]
public class NGUIDraggablePanel : IgnoreTimeScale
{
	...
	public NGUIScrollBar horizontalScrollBar;

	/// <summary>
	/// Vertical scrollbar used for visualization.
	/// </summary>

	public NGUIScrollBar verticalScrollBar;
	...
	public bool shouldMoveHorizontally
	{
		get
		{
			...
			if (mPanel.clipping == NGUIDrawCall.Clipping.SoftClip) size += mPanel.clipSoftness.x * 2f;
	...
	public bool shouldMoveVertically
	{
		get
		{
			...
			if (mPanel.clipping == NGUIDrawCall.Clipping.SoftClip) size += mPanel.clipSoftness.y * 2f;
	...
	void OnHorizontalBar (NGUIScrollBar sb)
	{
		if (!mIgnoreCallbacks)
		{
	...
	public void Press (bool pressed)
	{
		...
		if (enabled && NGUITools.GetActive(gameObject))
		{
			...
			{
				if (restrictWithinPanel && mPanel.clipping != NGUIDrawCall.Clipping.None && dragEffect == DragEffect.MomentumAndSpring)
				{
	...
	void LateUpdate ()
	{
		...
		if (mShouldMove && !mPressed)
		{
			...
			if (mMomentum.magnitude > 0.0001f)
			{
				...
				if (restrictWithinPanel && mPanel.clipping != NGUIDrawCall.Clipping.None) RestrictWithinBounds(false);
				return;
			}
			else
			{
