...
public class NGUIButtonColor : XUIActiveBase
{
	/// <summary>
	...
	/// <summary>
	/// NGUIButtonColor's default (starting) color. It's useful to be able to change it, just in case.
	/// </summary>
	...
	protected virtual void OnEnable () { if (mStarted && mHighlighted) OnHover(NGUICamera.IsHighlighted(gameObject)); }
    ...
    protected override void ProcessActive()
    {
        ...
        if (enabled) TweenColor.Begin(tweenTarget, duration, (NGUICamera.IsHighlighted(gameObject) ? hover : mColor));
    }
}
