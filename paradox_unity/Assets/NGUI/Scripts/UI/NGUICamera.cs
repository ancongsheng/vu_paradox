...
/// - OnClick () is sent with the same conditions as OnSelect, with the added check to see if the mouse has not moved much. NGUICamera.currentTouchID tells you which button was clicked.
/// - OnDoubleClick () is sent when the click happens twice within a fourth of a second. NGUICamera.currentTouchID tells you which button was clicked.
...
public class NGUICamera : MonoBehaviour
{
    ...
    /// <summary>
    /// How long of a delay to send holding events.
    /// </summary>

    public float holdingDelay = 1f;
	...
	static List<NGUICamera> mList = new List<NGUICamera>();
	...
	static public GameObject selectedObject
	{
		...
		set
		{
			if (mSel != value)
			{
				if (mSel != null)
				{
					NGUICamera uicam = FindCameraForLayer(mSel.layer);
					...
					if (uicam != null)
					{
                        ...
                        //reset the last reported keys events current.
	...
	{
		NGUIPanel panel = NGUITools.FindInParents<NGUIPanel>(hit.collider.gameObject);
	...
	/// <summary>
	/// Returns whether the widget should be currently highlighted as far as the NGUICamera knows.
	/// </summary>
	...
	public void ProcessOthers ()
	{
		...
		inputHasFocus = (mSel != null && mSel.GetComponent<NGUIInput>() != null);
	...
	public void ProcessTouch (bool pressed, bool unpressed)
	{
		...
		if (pressed)
		{
            ...
            mHoldingTime = Time.realtimeSinceStartup + holdingDelay;
		...
		else
		{
			...
			if (currentTouch.pressed != null)
			{
				...
				if (mag <= drag && mHoldingTime != 0f && mHoldingTime < Time.realtimeSinceStartup)
				{
					Notify(currentTouch.pressed, "OnHold", null);
					mHoldingTime = 0f;
				}
    ...
    public static void UpdatePressObject(GameObject obj)
    {
        currentTouch.pressed = obj;
        currentTouch.dragged = obj;
    ...
    //private static Event et = Event.KeyboardEvent(" ");
    //private static EventType ett = EventType.Layout;
    //private static bool etik = false;
    //private static KeyCode etk = KeyCode.Escape;
    ...
    void subFunc()
    {
        switch (Event.current.type)
        {
            case EventType.Repaint:
                ...
                //GUI.Label(new Rect(10, Screen.height - 2 * 20, Screen.width - 10, 100), "Key: " + ett + " " + inputHasFocus + " " + (Input.imeCompositionMode != IMECompositionMode.Off));


                //if (etik)
                //    GUI.Label(new Rect(10, Screen.height - 1 * 20, Screen.width - 10, 100), "Key: " + etk);
            ...
            default:
                ...
                //et = Event.current;
                //ett = et.type;
                //etik = et.isKey && ett == EventType.keyDown;
                //etk = et.keyCode;

                if (inputHasFocus && (Input.imeCompositionMode != IMECompositionMode.Off))
                {
                    //get last reported input			
                    if (Event.current.isKey && Event.current.type == EventType.keyDown && Event.current.keyCode == KeyCode.Backspace)
                    {
                        //CHECK IF BACKSPACE IS ALREADY IN THE INPUT STRING
                        if (lastInputEvents.LastIndexOf('\b') == -1 && Input.inputString.LastIndexOf('\b') == -1)
                        {
