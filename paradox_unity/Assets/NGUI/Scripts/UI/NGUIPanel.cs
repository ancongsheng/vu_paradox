...
public class NGUIPanel : MonoBehaviour
{
...
#if USE_SIMPLE_DICTIONARY
	Dictionary<Transform, NGUINode> mChildren = new Dictionary<Transform, NGUINode>();
#else
	OrderedDictionary mChildren = new OrderedDictionary();
#endif

	// List of all widgets managed by this panel
	BetterList<NGUIWidget> mWidgets = new BetterList<NGUIWidget>();
	...
	BetterList<NGUIDrawCall> mDrawCalls = new BetterList<NGUIDrawCall>();
	...
	public void SetAlphaRecursive (float val, bool rebuildList)
	{
		if (rebuildList || mChildPanels == null)
			mChildPanels = GetComponentsInChildren<NGUIPanel>(true);
	...
	public BetterList<NGUIWidget> widgets { get { return mWidgets; } }

	/// <summary>
	/// Retrieve the list of all active draw calls, removing inactive ones in the process.
	/// </summary>

	public BetterList<NGUIDrawCall> drawCalls
	{
		get
		{
...
#else
		if (t != null && mChildren.Contains(t)) node = (NGUINode)mChildren[t];
#endif
		return node;
	}
	...
	public bool IsVisible (Vector3 worldPos)
	{
		...
		if (mClipping == NGUIDrawCall.Clipping.None) return true;
		UpdateTransformMatrix();
	...
	/// <summary>
	/// Get or create a NGUIScreen responsible for drawing the widgets using the specified material.
	/// </summary>

	NGUIDrawCall GetDrawCall (Material mat, bool createIfMissing)
	{
        var dcCache = mDrawCalls;
		for (int i = 0, imax = dcCache.size; i < imax; ++i)
		{
			NGUIDrawCall dc = dcCache.buffer[i];
			if (dc.material == mat) return dc;
		}

		NGUIDrawCall sc = null;
		...
		if (createIfMissing)
		{
			...
			sc = go.AddComponent<NGUIDrawCall>();
			sc.material = mat;
			mDrawCalls.Add(sc);
		}
		return sc;
	...
	static BetterList<NGUINode> mHierarchy = new BetterList<NGUINode>();
	...
	void LateUpdate ()
	{
		...
		if (mLayer != gameObject.layer)
		{
            ...
            var cache = drawCalls;
            for (int i = 0, imax = cache.size; i < imax; ++i) mDrawCalls.buffer[i].gameObject.layer = mLayer;
		}
	...
	public void Refresh ()
	{
		NGUIWidget[] wd = GetComponentsInChildren<NGUIWidget>();
	...
	public Vector3 CalculateConstrainOffset (Vector2 min, Vector2 max)
	{
		...
		if (clipping == NGUIDrawCall.Clipping.SoftClip)
		{
