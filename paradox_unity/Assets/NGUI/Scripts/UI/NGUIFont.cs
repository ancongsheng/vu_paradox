...
/// <summary>
/// NGUIFont contains everything needed to be able to print text.
/// </summary>
...
public class NGUIFont : MonoBehaviour
{
	...
	[HideInInspector][SerializeField] NGUIAtlas mAtlas;
	...
	//add by chiuan for dynamic symbols used.
	[HideInInspector][SerializeField] NGUIFont mDynamicSymbolsFont;
	[HideInInspector][SerializeField] EmojiSpriteLabel mDynamicSymbolLabel;

    public EmojiSpriteLabel dynamicSymbolLabel
    {
        get
        {
            return (mReplacement != null) ? mReplacement.dynamicSymbolLabel : mDynamicSymbolLabel;
        }
        set 
        {
            if (mReplacement != null)
            {
                mReplacement.dynamicSymbolLabel = value;
            }
            else
            {
                mDynamicSymbolLabel = value;
            }
        }
    }

	// Cached value
	NGUIAtlas.Sprite mSprite = null;
	int mPMA = -1;
	...
	public bool hasSymbols { get { return (mReplacement != null) ? mReplacement.hasSymbols : symbols.Count != 0; } }
	...
	public List<BMSymbol> symbols { 
		get { 
			if(isDynamic == false)
				...
				return mDynamicSymbolsFont != null ? mDynamicSymbolsFont.symbols : mSymbols;
		} 
	}
	
	//add by chiuan 
	//add dynamic font used symbols.
	public NGUIFont dynamicSymbolsFont{
		get
		{
			return (mReplacement != null) ? mReplacement.dynamicSymbolsFont : mDynamicSymbolsFont;
		}
		set
		{
            if (mReplacement != null)
            {
                mReplacement.dynamicSymbolsFont = value;
            }
            else
            {
                mDynamicSymbolsFont = value;
                //ofcause need to refresh thoes mSymbols.
                if (mDynamicSymbolsFont != null) mSymbols = mDynamicSymbolsFont.symbols;
            }
		}
	}
	...
	public Material material
	{
		get
		{
			...
			if (isDynamic)
			{
				...
				mDynamicFont.material.shader = Shader.Find("Unlit/Dynamic Text Shader");
	...
	public NGUIAtlas.Sprite sprite
	{
		get
		{
	...
	public bool isDynamic { get { return (dynamicFont != null); } }
    ...
    /// <summary>
    /// Get or set the y offset of the dynamic font.
    /// </summary>
    ...
    public float dynamicFontOffset
    {
        get
        {
            return (mReplacement != null) ? mReplacement.dynamicFontOffset : mDynamicFontOffset;
        }
        set
        {
            if (mReplacement != null)
            {
                mReplacement.dynamicFontOffset = value;
            }
            else
            {
                if (mDynamicFontOffset != value)
                {
                    mDynamicFontOffset = value;
                    MarkAsDirty();
                }
            }
        }
	...
	void Trim ()
	{
		...
		if (tex != null && mSprite != null)
		{
			...
			Rect trimmed = (mAtlas.coordinates == NGUIAtlas.Coordinates.TexCoords) ?
	...
	public bool RecalculateDynamicOffset()
	{
		...
		{
			CharacterInfo j;
			mDynamicFont.RequestCharactersInTexture("|", mDynamicFontSize, mDynamicFontStyle);
			mDynamicFont.GetCharacterInfo('|', out j, mDynamicFontSize, mDynamicFontStyle);
	...
	public string GetEndOfLineThatFits (string text, float maxWidth, bool encoding, SymbolStyle symbolStyle)
	{
		...
		while (currentCharacterIndex > 0 && remainingWidth > 0)
		{
			...
			else
			{
				...
				else if (mDynamicFont.GetCharacterInfo(currentCharacter, out mChar, mDynamicFontSize, mDynamicFontStyle))
	...
	BMSymbol MatchSymbol (string text, int offset, int textLength)
	{
		...
		for (int i = 0; i < count; ++i)
		{
			...
			// Match found
			if (match && sym.Validate(dynamicSymbolsFont != null ? dynamicSymbolsFont.atlas : atlas)) return sym;
		}
		return null;
	}
