...
public class NGUISprite : NGUIWidget
{
	public enum Type
	{
		Simple,
    ...
    [HideInInspector][SerializeField] bool mFillReverse = false;
	...
	protected NGUIAtlas.Sprite mSprite;
    ...
    public bool fillReverse { get { return mFillReverse; } set { if (mFillReverse != value) { mFillReverse = value; MarkAsChanged(); } } }
	...
	override public Vector4 border
	{
		get
		{
            if (type == Type.Sliced || type == NGUISprite.Type.MirrorSliced)
			{
				NGUIAtlas.Sprite sp = GetAtlasSprite();
				...
				if (atlas.coordinates == NGUIAtlas.Coordinates.TexCoords && tex != null)
				{
    ...
    override public bool pixelPerfectAfterResize { get { return type == Type.Sliced || type == Type.MirrorSliced; } }
	...
	protected void SetAtlasSprite (NGUIAtlas.Sprite sp)
	{
		mChanged = true;
		mSpriteSet = true;
	...
	{
        if ((type == Type.Sliced || type == Type.Tiled || type == Type.MirrorSliced) && cachedTransform.localScale != mScale)
		{
	...
	/// <summary>
	/// Virtual function called by the NGUIScreen that fills the buffers.
	/// </summary>
	...
	override public void OnFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		switch (type)
		{
            ...
            case Type.MirrorSimple:
            MirrorSimpleFill(verts, uvs, cols);
            break;

            case Type.MirrorSliced:
            MirrorSlicedFill(verts, uvs, cols);
            break;
	...
	protected void TiledFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (atlas.coordinates == NGUIAtlas.Coordinates.TexCoords)
		{
    ...
    protected void MirrorSimpleFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
    {
        Vector2 uv0 = new Vector2(mOuterUV.xMin, mOuterUV.yMin);
        Vector2 uv1 = new Vector2(mOuterUV.xMax, mOuterUV.yMax);

        if (!mInnerUV.Equals(mOuterUV))
        {
            if (fillReverse)
            {
                uv0.x = mInnerUV.xMin;
        ...
        verts.Add(new Vector3(0.5f, -1f, 0f));
        verts.Add(new Vector3(0f, -1f, 0f));
        verts.Add(new Vector3(0f, 0f, 0f));
    ...
    protected void MirrorSlicedFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
    {
        if (mOuterUV == mInnerUV)
        {
            MirrorSimpleFill(verts, uvs, cols);
        ...
        if (tex != null)
        {
            ...
            if (fillReverse)
            {
                uv[0] = new Vector2(mInnerUV.xMin, mOuterUV.yMax);
                uv[1] = new Vector2(mInnerUV.xMin, mInnerUV.yMax);
                uv[2] = new Vector2(mInnerUV.xMax, mInnerUV.yMin);
                uv[3] = new Vector2(mOuterUV.xMax, mOuterUV.yMin);
            }
        ...
        for (int x = 0; x < 2; ++x)
        {
            ...
            for (int y = 0; y < 3; ++y)
            {
                ...
                // Mirror Left
                verts.Add(new Vector3(xMax, v[y].y, 0f));
                verts.Add(new Vector3(xMax, v[y2].y, 0f));
                verts.Add(new Vector3(xMin, v[y2].y, 0f));
                verts.Add(new Vector3(xMin, v[y].y, 0f));

                // Mirror Right
                verts.Add(new Vector3(1f - xMin, v[y].y, 0f));
                verts.Add(new Vector3(1f - xMin, v[y2].y, 0f));
                verts.Add(new Vector3(1f - xMax, v[y2].y, 0f));
                verts.Add(new Vector3(1f - xMax, v[y].y, 0f));
