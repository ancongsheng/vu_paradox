...
public class NGUIInput : MonoBehaviour
{
	...
	static public NGUIInput current;
	...
	NGUIWidget.Pivot mPivot = NGUIWidget.Pivot.Left;
	float mPosition = 0f;
	...
	public bool selected
	{
		get
		{
			return NGUICamera.selectedObject == gameObject;
		}
		set
		{
			if (!value && NGUICamera.selectedObject == gameObject) NGUICamera.selectedObject = null;
			else if (value) NGUICamera.selectedObject = gameObject;
		}
	}
	...
	protected void Init ()
	{
		...
		{
			mDoInit = false;
			if (label == null) label = GetComponentInChildren<NGUILabel>();
	...
	void OnEnable () { if (NGUICamera.IsHighlighted(gameObject)) OnSelect(true); }

	/// <summary>
	/// Remove the selection.
	/// </summary>

	void OnDisable () { if (NGUICamera.IsHighlighted(gameObject)) OnSelect(false); }

	/// <summary>
	/// Selection event, sent by NGUICamera.
	/// </summary>
		...
		if (label != null && enabled && NGUITools.GetActive(gameObject))
		{
			if (isSelected)
			{
...
#endif
				{
					...
					Input.compositionCursorPos = NGUICamera.currentCamera.WorldToScreenPoint(offset);
				}
				UpdateLabel();
			}
			else
	...
	void Update ()
	{
		if (selected)
		{
			...
			{
				NGUICamera.selectedObject = selectOnTab;
			}
	...
	/// <summary>
	/// Input event, sent by NGUICamera.
	/// </summary>
	...
	void UpdateLabel ()
	{
		...
		if (label.font != null)
		{
			...
			if (!label.shrinkToFit)
			{
				if (label.multiLine)
				{
					processed = label.font.WrapText(processed, label.lineWidth / label.cachedTransform.localScale.x, 0, false, NGUIFont.SymbolStyle.None);
				}
				else
				{
					string fit = label.font.GetEndOfLineThatFits(processed, label.lineWidth / label.cachedTransform.localScale.x, false, NGUIFont.SymbolStyle.None);
					...
					if (fit != processed)
					{
						...
						if (mPivot == NGUIWidget.Pivot.Left) label.pivot = NGUIWidget.Pivot.Right;
						else if (mPivot == NGUIWidget.Pivot.TopLeft) label.pivot = NGUIWidget.Pivot.TopRight;
						else if (mPivot == NGUIWidget.Pivot.BottomLeft) label.pivot = NGUIWidget.Pivot.BottomRight;
